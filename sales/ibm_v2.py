...
def create_styled_excel_v2(
	...
	for idx, row in enumerate(data, start=1):
		...
		if country == "Qatar":
			# row: [SKU, Product Description, Quantity, Start Date, End Date, Unit Price in USD, Cost (USD), Partner Discount, Partner Price in USD]
			...
			ws.cell(row=excel_row, column=7).number_format = '"USD"#,##0.00'  # Unit Price in USD
			ws.cell(row=excel_row, column=8).number_format = '"USD"#,##0.00'  # Cost (USD)
			ws.cell(row=excel_row, column=9).number_format = '"USD"#,##0.00'  # Partner Discount
	...
	if country != "Qatar":
		...
		# For Qatar, summary_row is not used, but set to a safe value for downstream logic
		summary_row = start_row + len(data) + 1 if data else start_row + 1
