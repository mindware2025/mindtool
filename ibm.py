...
def extract_last_page_text(file_like) -> str:
    doc = fitz.open(stream=file_like.read(), filetype="pdf")
    last_page = doc[-1]
    return last_page.get_text("text") or last_page.get_text()
# ----------------------------------------------------------------------
# Excel creation
# ----------------------------------------------------------------------
def create_styled_excel(
    ...
    ws.column_dimensions[get_column_letter(4)].width  = 60  # D (Description)
    ws.column_dimensions[get_column_letter(5)].width  = 12  # E (Qty)
    ws.column_dimensions[get_column_letter(6)].width  = 16  # F (Start Date)
    ws.column_dimensions[get_column_letter(7)].width  = 16  # G (End Date)
    ws.column_dimensions[get_column_letter(8)].width  = 18  # H (Unit Price AED)
    ws.column_dimensions[get_column_letter(9)].width  = 18  # I (Total Price AED)
    # Left block
    ...
    def estimate_line_count(text, max_chars_per_line=90):
        lines = text.split('\n')
        total_lines = 0
        for line in lines:
            if not line:
    ...
    for cell_addr, text, *style in terms:
        ...
        # Height by estimated wrap
        line_count = estimate_line_count(text, max_chars_per_line=90)
    ...
    # Divider line across current header row if needed
    border_row = 4
    ...
    last_row = ws.max_row
    ws.print_area = f"{get_column_letter(first_col)}1:{get_column_letter(last_col)}{last_row}"
...
# --- Set page setup for landscape and fit to 1 page width ---

# --- Optional: set small margins to help fit ---

# --- Optional: force scale if still cutting ---
    ws.page_setup.scale = 75  # percent (adjust 70–90 if needed)
    wb.save(output)
